...
const renderer = new THREE.WebGL1Renderer({   antialias: true, alpha: true, powerPreference: "high-performance"})
...
const renderer2 = new THREE.WebGL1Renderer({  antialias: false})
...
const camera2 = new THREE.PerspectiveCamera(45, containerDetails.clientWidth / containerDetails.clientHeight, 1, 100)
camera2.position.set(4.5,1.9,2.3)
camera2.rotation.set(0.1,1.0,-1.1)
scene.add(camera2)
...
function introAnimation() {
    new TWEEN.Tween(camera.position.set(0,4,2.7)).to({ x: 0.2, y: 0.7, z: 8.1}, 3500).easing(TWEEN.Easing.Quadratic.InOut).start()
    .onComplete(function () {
        TWEEN.remove(this)
...
document.getElementById('automation').addEventListener('click', () => {
    document.getElementById('automation').classList.add('active')
    document.getElementById('education').classList.remove('active')
    document.getElementById('sap').classList.remove('active')
    document.getElementById('content').innerHTML = 'Industrial automation systems, Industry 4.0 transformations, Machinery, plant, energy systems and more…'
    animateCamera({ x: 1.9, y: 1.7, z: 2.1 },{ y: 1.1 })
})

document.getElementById('sap').addEventListener('click', () => {
    document.getElementById('sap').classList.add('active')
    document.getElementById('automation').classList.remove('active')
    document.getElementById('education').classList.remove('active')
    document.getElementById('content').innerHTML = "The company's production planning, financial analysis, production analysis and many more data analysis solutions…"
    animateCamera({ x: -0.9, y: 1.1, z: 3.1 },{ y: -0.1 })
})
...
document.getElementById('education').addEventListener('click', () => {
    ...
    document.getElementById('automation').classList.remove('active')
    document.getElementById('sap').classList.remove('active')
    document.getElementById('content').innerHTML = 'Automation systems trainings, training sets, online support…'
    animateCamera({ x: -1.9, y: 2.7, z: 2.9 },{ y: -0.6 })
})
